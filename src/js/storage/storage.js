...
export const watch = [].concat(
  JSON.parse(window.localStorage.getItem('watched')) ?? []
); // база просмотреных
export const queue = [].concat(
  JSON.parse(window.localStorage.getItem('queued')) ?? []
); // база в очереди

// console.log(auth);

const funcUnic = (obj, id, list) => {
  // функция проверки обьекта на уникальность
...
export const funcAddWatchBtn = (obj, btnW, btnQ, id) => {
  // функция добавления на страницу библиотекы данных

  btnW.addEventListener('click', () => {
    // в список просмотреных
